...
| **v15 结构同族（路线 A）** | 🟡 **进行中**：结构侧 P0–P4 增量 6 ✅（指纹 16/32）；安全增量 S1 ✅（密钥流状态机化）；**阶段 C 首尝试已回滚**（2026-08-28：每指令独立 handler 的 CPS 分发有「每调用 +1 帧」固有栈代价，`tail(5000)` 深尾递归栈溢出——我们尾调用是模拟非真 TCO；已回滚回内联分发，204/204+405/405 全绿。栈代价解法待决策：① 内联 Call 系 opcode ② 真 TCO/帧复用。详见 `docs/v15-pipeline-rewrite.md` 阶段 C 注记） |
