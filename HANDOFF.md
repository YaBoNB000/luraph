...
| **v15 结构同族（路线 A）** | 🟡 **进行中**：P0 ✅ → P1 ✅ → P2 ✅ → P3 增量 1–5 ✅ → **P4 增量 6 ✅**（载体密钥流混淆：分块构建期 `(K1*i+K2)%256`+bxor 加密、运行时解回，存储字面量不再可直接 base-94 解码；F16 PASS；指纹 16/32；F8 长串与 XOR 互斥的架构注记已记录）；**下一步：执行循环真正内联进运行器**（F11/F12/F5 大头），细节在 `docs/v15-structural-parity-plan.md` |
