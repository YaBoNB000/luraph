...
>
> **最后更新：2026-09-16**（**增量㉝ 词表挪进长字符串**：用户拍板「挪进去」
> ——P3b 十进制数字槽词表（~11.5B/词）改 5 字符/词连续区 base-94 长字符串
> （解码纯 Horner 无查表），掩码/分片机制不变；R016 443KB→409KB。诚实
> 解剖：词表仅 ~108KB，体积大头 = 每载体 5 段 staging handler 内联代码
> （~260KB，拟态设计产物），继续压要动结构参数待拍板。门禁 166/166 +
> 360/360 + 多种子 0 + 轮廓 32/32 + 安全 5/5 + 钥匙 0 泄漏（R016 71 组）
> + 字节级确定。
> 前序：㉜ R016 大载荷（真实逻辑靶标 + VM 多赋值修复 + 槽池扩容）→
> ㉛ R014 回合三刀 → ㉚ B-2 源码哈希链 → ㉙ 反 trace 计时守卫 → ㉘ 净化 →
> B-3 按块解码 → ㉗ 运行时入密 → ㉖ Luau-only → ㉕ 默认即 v15 → ㉔ 守卫钥匙化）
