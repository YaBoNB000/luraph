...
| **「看着像+安全性也像」战役** | ✅ **完成（2026-09-05）**：三个致命缺点全部闭环，安全指纹 **0/5 → 5/5**。P0 安全指纹 S1–S5 + 攻击脚本；P1 常量加密+动态内联（S1）；P2 字节码不规则化：节化乱序 + 位置掩码 + 假节 + 流加噪（S2）；P3a handler 体加密 + loadstring 原生复检引导 + CPS 信号化（S3/S5）；P3b OC 运行时生长 + BW 拆槽掩码 + LCG 变形（S4）；P3c 口径定稿 + 诱饵分发点（flatten 实测回退：样本 VM 内核是循环分发非扁平状态机）。双脚本门禁：`v15_fingerprint.py`（轮廓 32 条）+ `security_fingerprint.py`（安全 5 项），改 `vmgen/` 发射面必跑。计划 `docs/plan-resemblance-and-security.md` |
