...
| **「看着像+安全性也像」战役** | 🚧 **进行中（2026-09-05 立项）**：三个致命缺点整改；P0 完成（安全指纹 S1–S6 + 攻击脚本，基线 0/5 全红：常量 96% 恢复/字节码规整熵 4.5/解释器裸露/输出自描述）；**P1 完成（常量加密+动态内联：ckseed+LCG 掩码 / dyadic 数字 / MkStr 短串内联，S1 转绿 0 恢复）**；待做 P2 字节码不规则 → P3 解释器分层。计划 `docs/plan-resemblance-and-security.md` |
