...
| **v15 结构同族（路线 A）** | 🟡 **进行中**：P0 ✅ → P1 ✅ → P2 ✅ → P3 增量 1–3 ✅ → **增量 4 ✅**（编译器 `nop_sites` 位置表 → 模板定值自修改写回 F14=76/F27=19；打印机语句间 `;`；mangle v15 命名族 1–2 字符；v15 关 numbers——数值拆分会毁字面量形态；指纹 14/32）；**下一步：执行循环真正内联进运行器**（F11 随之扩量），细节在 `docs/v15-structural-parity-plan.md` |
