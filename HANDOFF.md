...
>
> **最后更新：2026-09-17**（**增量㉟ 自研压缩——碎片增量链**：HBOOT 解码链
> 即压缩链，每片对「此前已解码源码字典」做 4-gram 贪心 LZ 增量编码，字典
> 逐片生长，加密层不变，debug 构建带往返自检。正确性：往返断言 0 失败 +
> 链终值逐位一致 + 166/166 + 360/360 + 轮廓 32/32 + 安全 5/5 + 钥匙清洁。
> **⚠️ 已知待修**：排查中发现 v15 引导层稀有（~0.3%）种子依赖潜在 bug
> （确定性「attempt to call a number/table」，legacy --vm 全过，先于本增量），
> 已排除 MP 掩码/块编解码/链重建/ck 校验/staging fold；标准 multiseed 当前
> 命中 basics seed=1，列为下一优先专项。
> 前序：㉞ 一趟流水融合 → ㉝ 词表长字符串化 → ㉜ R016 大载荷回合 → ㉛ R014
