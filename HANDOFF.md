...
>
> **最后更新：2026-09-16**（**增量㉜ R016 大载荷回合**：用户拍板 P2-5，靶标
> 换真实逻辑载荷（地形噪声 + A* 寻路 + 生态模拟 + 中继路由，~430 行，混淆
> 输出 443KB）。大载荷暴露并修复两个真问题：① VM 多赋值语义 bug（求值/
> 存储交错违反 RHS 先全量求值，同表交换读到已覆写槽）→ 统一存储修复 +
> 新语料 swap_multi 锁回归；② 模块表槽池样本尺度被大载体占满 → 槽池扩容
> + 原语抽取上界放开。门禁 166/166 + 360/360 + 多种子 0 + 轮廓 32/32 +
> 安全 5/5 + 钥匙 54 组 0 泄漏 + 同种子字节级确定。
> 前序：㉛ R014 回合三刀 → ㉚ B-2 源码哈希链 → ㉙ 反 trace 计时守卫 →
> ㉘ 碎片源码净化 → B-3 按块解码 → ㉗ 运行时入密 → ㉖ Luau-only → ㉕ 默认即 v15 → ㉔ 守卫钥匙化）
