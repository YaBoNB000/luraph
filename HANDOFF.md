...
| **v15 结构同族（路线 A）** | 🟡 **进行中**：P0 ✅ → P1 ✅ → P2 ✅ → P3 增量 1/2 ✅ → **P3-B 增量 3 ✅**（Nop 别名自修改：装载期字节码自变异、双 Nop 分发叶、OC.Nop/NopA 双活；死循环段；解释器迁入数字槽运行器 [r1] + 入口经 [r2] 路由，函数槽 4 个；指纹 13/32；**legacy 预设输出逐字节零变化**）；**下一步：执行循环真正内联进运行器 + 字面量自修改 + 分号发射**，细节在 `docs/v15-structural-parity-plan.md` |
