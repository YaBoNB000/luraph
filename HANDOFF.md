...
| **Rust 混淆器代码** | ⬜ **未开始，等用户说「开始」** |

**如果用户说「开始」**，按 §7 的顺序动工。建议第一个里程碑：
Rust 骨架 + lexer + parser + AST + 打印器，做到「输入 → 输出语义等价
（round-trip）」并通过 §2.2 的双解释器验证，再逐层加混淆 pass。
...
→ L3 控制流（for/repeat 去糖 → CFG 扁平化状态机，无 goto 的 5.1 安全形态）
...
4. **里程碑 A**：round-trip（输入 → AST → 输出，语义等价）+ 全部测试语料
   过双解释器 → 给用户看
5. 逐个 pass：`desugar` → `mangle`(L1) → `strings`(L2) → `flatten`(L3)
   → `junk` → `numbers`(L4) → `body`(L5) → `antidbg`(L7)
   —— **每加一个 pass 都执行 §2.2 强制工作流**
