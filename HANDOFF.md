...
>
> **最后更新：2026-09-19**（**增量㊴ 激活门退役**：`--bind-key` 输入
> 绑定整体移除——功能代码（template.rs 门控发射/激活哈希/`LURAPH_BIND_DBG`、
> main.rs 选项校验、v15.rs 变长参透传）+ 测试（`bind_gate_test.sh`、
> multiseed 绑定扫描、审计 D7）+ 绑定态示例全部清除。移除零影响证明：
> 36 个未绑定示例与移除前逐字节一致 + 156/156 + 360/360 + multiseed
> 0 失败 + 审计 16/16。原靶标更名 `real_shift.lua` 留作语料；环境绑定
> 不受影响。前序：㊳ 计时守卫复通（㉙ 引入、㊱ 因作用域
