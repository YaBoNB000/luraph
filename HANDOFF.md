...
| **v15 结构同族（路线 A）** | 🟡 **进行中**：P0 ✅ → P1 ✅ → P2 ✅ → P3 增量 1–4 ✅ → **增量 5 ✅**（顶层机族扩容：FC → 子分发器 → CPS 循环三层分发同构样本 + 2 附加机初始化器，F32 PASS；指纹 15/32）；**下一步：执行循环真正内联进运行器**（F11/F12/F5 扩量）+ P4 blob，细节在 `docs/v15-structural-parity-plan.md` |
