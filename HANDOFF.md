...
| **v15 结构同族（路线 A）** | 🟡 **进行中**：结构侧 P0–P4 增量 6 ✅（指纹 16/32）；**用户优先级：安全 > 结构** → **安全增量 S1 ✅**（密钥流状态机化：载体 XOR 密钥从明文常量改为样本 `[96]` 同款 3 步 LCG 状态机，解码点无密钥常量、静态不可提取；30 语料 + 多种子 5×8 + 矩阵 204 + 预设 405 全绿）；**下一步：阶段 C 执行 CPS 化（安全+结构双收益）或安全项 S2 操作数 7-bit 分块不透明化**，安全对账表在 `docs/v15-pipeline-rewrite.md` §6 |
