...
> `tests/defense_audit.sh`，13 个防御点逐一正/负向实测，**15 PASS /
> 0 FAIL（ALL DEFENSES ACTIVE）**。覆盖：干净运行等价、钥匙去字面化
> （3 种子×52 钥匙 0 泄漏）、结构指纹 32/32、安全指纹 S1–S5 5/5、
> 密文完整性+字节码防篡改（30 个真篡改点 0 次正确输出）、激活门
> 10/10、环境绑定 6/6、反挂钩闸（loadstring 换 Lua 闭包 → 被杀）、
> 反指纹闸（debug.info 说谎 → 被杀）、蜜罐哨兵结构、计时守卫确认
> 显式关闭且可复通、LZ 往返自检（debug 构建）。附修审计空互换盲点。
...
> 该作用域 bug 静默禁用，现显式保持关（启用需评估误报，待用户拍板）。
