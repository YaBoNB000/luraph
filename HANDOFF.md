...
| **v15 结构同族（路线 A）** | 🟡 **进行中**：P0 ✅ → P1 ✅（外壳 3 行）→ P2 ✅（65 原语槽 + 诱饵 LCG + 具名常量）→ P3 增量 1 ✅（CPS 骨架）→ **P3 增量 2 ✅**（每原型 = 字节和折叠 + 3 分块存储共 4 个单字符 handler；定义/入口分离；校验阶段重折叠比对，篡改 → 静默死循环，实测生效；`functions` 语料 157 字段/78 具名函数/74 returns，指纹 12/32）；**下一步 P3-B 执行内联**（双运行器多段内联树 + 自修改 + 死路径），细节在 `docs/v15-structural-parity-plan.md` |
