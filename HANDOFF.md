...
>
> **最后更新：2026-09-10**（**选项B·B-3 状态链式按块解码**：基本块
> 分割 + 会话盐按块编码（静态文件无解码材料）+ 按块即需即解（帧私有
> 窗口+块缓存）——批量解码器消亡，文件级/单次转储级/跨运行重放级解码
> 全灭。门禁 161/161 + 350/350 + 多种子 0 + 轮廓 32/32 + 安全 5/5 +
> 钥匙 33 组 0 泄漏；fib(25) 2.57s。前序：㉗ 运行时入密 → ㉖ Luau-only
> → ㉕ 默认即 v15 → ㉔ 守卫钥匙化
> （废布尔标志/死循环，探测结果折进元钥匙流）+ 环境值绑定（目标运行时
> 分量值揉进钥匙流，哑桩即错钥）+ 新语料 token_mix 换源防污染。官方
> 矩阵 265/265 + 预设 480/480 + 多种子 0 失败 + 轮廓 32/32 + 安全 5/5 +
> 钥匙 38 组 0 泄漏；R010 双形态靶标（新源、seed 910）已交付。
> 前序：㉓ 解码窗口入密 → ㉒ 碎片即用即毁 → ㉑ 环境绑定 → ⑲⑳ → ⑱ 弃用）
...
门禁 265/265 + 480/480 全绿 + 钥匙 38 组（含绑定态）0 泄漏。剩余口子：
活帧碎片（调试器等价物）；常驻解码器+密文树同时被深度转储仍可离线解码
（B-3 状态链式按块解码候选）。计划
