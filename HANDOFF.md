...
>
> **最后更新：2026-08-28**（v15 阶段 A 完成：操作数散布——寄存器/常量/
> upvalue 随机槽，安全项全部收官；官方 204 + 预设 405 + 多 seed 5×8 全绿）
...
| **v15 结构同族（路线 A）** | 🟡 **进行中**：结构侧增量 1–6 ✅ + 安全增量 S1 ✅ + **阶段 C ✅ 执行 CPS 化**（真 TCO + 每指令 `H[OC.*]` handler + **Call 系内联**，`tail(5000)` 不溢栈）+ **阶段 A ✅ 操作数散布**（2026-08-28：寄存器/常量/upvalue 每函数随机槽散布——单寄存器操作数直写散布值、范围型操作经 blob 内槽表 S 翻译、makefn 经父帧 S 翻译 upsrc、**溢出表 O** 解决 nres=255 越界；静态寄存器恢复失锚，随机面全在载体编码 blob 内；单测 `scatter_layout_properties`）；全量回归绿（204/204 + 405/405 + 多 seed 5×8），指纹 17/32。**下一步：阶段 D（每帧闭包，高风险）/阶段 E（指纹量级收口）——均结构打磨，安全项已收官**。详见 `docs/v15-pipeline-rewrite.md` |
