...
>
> **最后更新：2026-09-14**（**增量㉚ B-2 源码自校验哈希链（硬版本）**：
> 破解 AI 方案落地——解释器自身源码哈希直接参与密钥派生。HBOOT 在
> loadstring 每片之前握着其明文源码（不需 debug.info/文件回读，㉙「自哈希
> 不可行」结论被绕过）：对 HBOOT 自身源码 + 53 碎片解码态源码走双车道
> 非线性链；每片钥匙种子揉入此前全部源码的链值（篡改 ⇒ 后续全片钥匙错位，
> 零比较语句）；链终值交 RT 碎片做双独立归零约束（指令块 + 常量池钥匙流
> 各一条，~52bit 二预映像）——源码被改则程序照跑、解密内容全错，无死循环
> /布尔标志可补（BSS 校验和惩罚同步从明文死循环改块步长毒化）。每片
> +14–22B 随机尾注。门禁 162/162 + 350/350 + 多种子 0 + 轮廓 32/32 + 安全
> 5/5 + 钥匙 46 组 0 泄漏 + 同种子字节级确定。前序：㉙ 反 trace 计时守卫 →
> ㉘ 碎片源码净化 → B-3 按块解码 → ㉗ 运行时入密 → ㉖ Luau-only →
> ㉕ 默认即 v15 → ㉔ 守卫钥匙化 → ㉓ 解码窗口入密 → ㉒ 碎片即用即毁 → ㉑ 环境绑定）
