...
>
> **最后更新：2026-09-13**（**增量㉙ 反 trace 计时守卫 + 攻击方四问题
> 收口**：自适应基线计时守卫（前 4 窗校准、超基线 100 倍连续 2 窗→污染
> 块钥匙流错钥死）回应「逐指令插桩无察觉」；并收口攻击方四条——整 chunk
> 自哈希实证不可行（无 io、debug.info 只回预览）/预编译字节码被平台禁止
> （㉘ 为其替代）/89621 探针职责澄清。门禁 162/162 + 350/350 + 多种子 0
> + 轮廓 32/32 + 安全 5/5。前序：㉘ 碎片源码净化 → B-3 按块解码 →
> ㉗ 运行时入密 → ㉖ Luau-only → ㉕ 默认即 v15 → ㉔ 守卫钥匙化 → ㉓ 解码窗口入密 → ㉒ 碎片即用即毁 → ㉑ 环境绑定）
