...
| **「看着像+安全性也像」战役** | 🚧 **进行中（2026-09-05 立项）**：三个致命缺点整改；P0 完成（安全指纹 S1–S6 + 攻击脚本）；**P1 完成（常量加密+动态内联，S1 转绿 0 恢复）**；**P2 完成（字节码不规则化：节化乱序 + 位置掩码 + 假节 + 流加噪，S2 转绿，熵 4.55→7.31）**；**P3a 完成（handler 体加密 + loadstring 引导：42 handler 密文化、原生复检、CPS 信号化、每帧环境 E，S3/S5 转绿，安全指纹 4/5）**；待做 P3b（OC/AL/TK/BW 去物化 → S4）+ P3c（骨架强化 + 指纹回校）。计划 `docs/plan-resemblance-and-security.md` |
