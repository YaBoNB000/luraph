...
| **「看着像+安全性也像」战役** | 🚧 **进行中（2026-09-05 立项）**：三个致命缺点**全部闭环**；P0 完成（安全指纹 S1–S6 + 攻击脚本）；**P1 完成（常量加密+动态内联，S1 转绿）**；**P2 完成（字节码不规则化：节化乱序 + 位置掩码 + 假节 + 流加噪，S2 转绿）**；**P3a 完成（handler 体加密 + loadstring 引导，S3/S5 转绿）**；**P3b 完成（OC 运行时生长 + BW 拆槽掩码 + LCG 变形，S4 转绿，安全指纹 8 语料 5/5）**；待做 P3c（可见骨架过 flatten/数值混淆 + 指纹口径回校）。计划 `docs/plan-resemblance-and-security.md` |
