...
| **v15 结构同族（路线 A）** | 🟡 **进行中**：安全项全部收官（S1 密钥流 + 阶段 B 解码状态机 + 阶段 C 执行 CPS + 阶段 A 操作数散布）。**2026-08-29 用户拍板：先把结构 100% 还原（32/32 指纹）再回头搞安全**。结构战役 **增量 E1 ✅ 指纹 17→27/32**：复合赋值折叠（打印器，纯左值门控）+ CHUNKS 3→5 + 融合条件诱饵 + fetch 形扫描循环 + `=N,function(...)` 包装 + 高位诱饵 LCG 槽 + 元组槽 `b[J[4]]/b[K[7]]` 路由 + 命名字段写。**增量 E2 ✅ 指纹 27→29/32**：F6 宽参数（状态元组 3→5 填充槽、staging handler 全 7 参、CPS 穿透 `f1..f5`，1→109）+ F22 Luau if 表达式（解析器新增 `Expr::IfExpr`，打通 clone/symtab/numbers/打印器/VM 编译器，0→18）。**剩余 3 条**：F8 RC 长串 + pC 表、F10 字符串隐匿（233→≤60）、F13 内联 -128 阶梯。全量回归绿（204/204 + 405/405 + 多 seed）。详见 `docs/v15-pipeline-rewrite.md` + implementation-plan 更新日志 |
