# L6 VM 实现笔记（M4 初版 2026-08-24 + 续期加固 2026-08-25）

> M4「VM 最小可用」落地记录：架构、语义模型、以及调试过程中踩出的
> **全部关键坑**（每个坑都有真实语料复现）。M4 续期（§8）把语料扩到
> 29 个后暴露并修复 9 类语义 bug——**现行矩阵 204/204 全绿 + 多种子
> 回归 0 失败**；upvalue 模型已从 §2 的「值副本+写回转发+快照」换代为
> §8.1 的**单 cell 别名模型**（§2 保留作演进记录）。
> 后续 M5（随机面收尾，清单见 §7）/ v2 改代码前必读本文件。
...
剩余：无（M5 于 2026-08-25 收官）。

- [x] SoA 平行数组容器（`[ncode][W bytes][S0..S3 varint]`；pc 按指令
      步进；跳转 = 1 基指令下标。码流数组名必须是 `W` 不能叫 `OC`）
- [x] 7-bit 完整档（7/14/21/28-bit + 128 进制重建 + 2³² 归一化）
- [x] 解码枢纽/状态元组位置每构建随机（inline / `hub()`；6 元组序；
      `run` 字段序；u16/r16/decarrier 声明序。`AL`/`TK` 必须在
      `decarrier` 之前声明）
- [x] base-94 + 保留前缀 token 转义（v15 pC 10 特殊字符；4 字节 LE
      长度前缀）
- [x] 帧运行器入场原语解包（15 原语 → `P[1..80]` 随机槽，顶层 + run）
- [x] 反编译人工抽查（`luac51 -l` 仅见 L5 容器；用户串不可检索；
      同 seed 逐字节一致 / 异 seed 前 2KB 重合 9.5%）
