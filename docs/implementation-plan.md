...
> 活文档：**每完成一个里程碑必须同步更新 §1 表格状态列 + §2 进度表 + §3 里程碑
> 勾选 + 更新日志**（四处一起改，不允许只记日志不改状态）。
> 初版：2026-08-23 ｜ 最后更新：2026-08-23（M1 完成后）
> **当前状态：M0 ✅ + M1 ✅（L1+L2 已实现），下一步 M2 控制流**
...
| minify / 空白压缩 | 输出层控制（单行/紧凑） | `printer.rs` | P0 | 🟡 规范化输出已有（M0），单行 minify 模式未做 |
...
| CFG 扁平化（函数级状态机） | 块图 → 随机状态 ID + 随机分支序 + `while true` 分派（无 goto 形态） | `flatten.rs` | P0 | ⬜（M2，下一步） |
...
| 预设 | low(L1+L2) / medium(+L3) / high(+L4+L5+L7) / vm(全开) / max(v2 特性) | `main.rs` | P0 | 🟡 单开关已有（--no-mangle/--no-strings，M1），预设命名待 M6 |
| 强制测试工作流 | 每 pass 完成 → 混淆样本 → lua51+luau 语法+运行对比（语料 17 个文件） | `run_tests.sh`+`gen_examples.sh` | P0 | ✅ M0 起执行中（当前 62 项检查全绿） |
...
| **Rust 混淆器代码** | ✅ **M0+M1（2/7 里程碑）** | M0 地基（lexer/parser/symtab/printer）+ M1 混淆（L1 mangle + L2 strings） |
| 测试语料 + 矩阵 | ✅ 100% | 17 个语料文件（12 类语法 + 5 个 Luau 专属）；62 项检查全绿 |
| 混淆示例 | ✅ M1 产出 | `luraph-rs/examples/`（19 个文件，`tests/gen_examples.sh` 再生成） |

**一句话：设计 100%，代码 M0+M1 完成（2/7 里程碑，矩阵 62/62 全绿），下一步 M2 控制流扁平化。**
...
| **M2 控制流** ⬜（下一步） | desugar + flatten + junk(L3) | 同上；人工抽查输出为状态机形态、无原结构残留 |
