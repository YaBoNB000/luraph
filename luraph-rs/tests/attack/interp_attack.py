...
S3 判定（指令语义可见）：寄存器索引形态 `[x + y]` 密度 + 4 参函数数
   ——解释器被加密后两者应骤降。
S4 判定（自描述消除）：u32 字表 / LCG 生成器 / AL 94 项表 / TK 10 项表 /
   大型 名→数 映射表 —— 能被静态重建的素材越少越好。
...
def main():
    ...
    print(f"S4 自描述素材: BW {len(bw)} 词 / LCG {'有' if lcg else '无'} / "
          f"AL {'有' if alpha else '无'} / TK {'有' if tokens else '无'} / "
          f"OC 形大表 {oc_like} 个 -> "
