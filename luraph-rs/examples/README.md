...
**增量⑱ 输入绑定/激活门演示样本**：`bind_gate.lua`（真实逻辑门控靶标）
+ `bind_gate.v15.luau.lua`（未绑定，`--preset v15`）+
`bind_gate.bound.v15.luau.lua`（**绑定产物**，`--bind-key luraph-2026`）。
绑定产物把引导链第一层（HBOOT 元钥匙流）门控到激活值上：递送正确激活值
才解码执行，错误/缺失则 `loadstring` 得 nil 崩溃、字节码全程未解码。
激活值须以**第一个变长参数**递送（沙箱 Luau CLI 不转发命令行参数、全局
只读、无 io，这是唯一通道）——参考包装见 `tests/bind_gate_test.sh` 的
`mkwrap`/`mkls`。此处 `luraph-2026` 仅为**演示值**（真实部署勿把激活值
入仓）。绑定产物经 10 项门控验证（正确/错误/缺失钥匙、激活串 0 残留、
三种子、两种交付形态）。**增量⑳ 起激活哈希 = 双因子非线性**
（`bit32` xor/lrotate/乘加三混 ×2 组参数，扰动项翻倍），较⑱ 的线性
fold31 收窄代数/字典攻击面；激活值建议用授权码级长随机串。
