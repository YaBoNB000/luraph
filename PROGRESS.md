...
> 最后更新：2026-09-13（**增量㉙ 反 trace 计时守卫 + 攻击方四问题收口**：
> 自适应基线计时守卫（前 4 窗校准取最小、后续超基线 100 倍且连续 2 窗→
> 污染块钥匙流错钥静默死）回应「逐指令插桩拖慢几个数量级无察觉」；并对
> 攻击方四条逐一收口——整 chunk 运行时自哈希经实证在 Luau 沙箱原理不可行
> （无 io、debug.info(f,'s') 只回截断预览），预编译字节码被 Luau 安全模型
> 禁止（㉘ 净化为其替代），89621 探针职责澄清为 loader 钩子检测、环境区分
> 归 ㉑。门禁 162/162 + 350/350 + 多种子 0 + 轮廓 32/32 + 安全 5/5。
> 前序：㉘ 碎片源码净化 → B-3 按块解码 → ㉗ 运行时入密 → ㉖ Luau-only
