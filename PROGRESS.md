...
> 指纹 17/32，全语料 30 个 v15 示例入库；**用户优先级「安全 > 结构」
> 下的全部安全项已收官**，剩余为阶段 D（每帧闭包）/阶段 E（指纹量级
> 收口）的结构打磨，见 `docs/v15-pipeline-rewrite.md`
...
### 🟡 v15 结构 100% 还原战役（2026-08-29 起，用户拍板：先结构后安全）
...
**剩余 5 条**：F6 宽参数 handler、F8 RC 长串 + pC 表、F10 字符串隐匿
（233→≤60）、F13 内联 -128 阶梯、F22 if 表达式（需解析器扩展）。
