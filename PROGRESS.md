...
> 最后更新：2026-09-14（**增量㉚ B-2 源码自校验哈希链（硬版本）——哈希即
> 钥匙**：破解 AI 方案落地。HBOOT 在 loadstring 前握着每片明文源码——不靠
> debug.info/文件回读，直接对「HBOOT 自身源码 + 全部 53 碎片解码态源码」
> 走双车道非线性链：① 每片钥匙种子揉入此前全部源码的链值（篡改 ⇒ 后续
> 全片钥匙错位，链上零比较语句）；② 链终值 (H1,H2) 交 RT 碎片，双独立
> 归零约束分别揉进指令块钥匙流与常量池钥匙流——源码完整则偏移为 0 原样
> 跑，任一层被改则程序照跑、解密内容全错（~52bit 二预映像难度，期望哈希
> 不落明文常量）；③ 每片 +14–22B 随机尾注（自由填充位 + 构建熵）；附带
> BSS 校验和惩罚从明文死循环改为块步长毒化。㉙「整 chunk 自哈希不可行」
> 的结论被本路径绕过（无需回读自身）。门禁 162/162 + 350/350 + 多种子 0
> + 轮廓 32/32 + 安全 5/5 + 钥匙 46 组 0 泄漏 + 同种子字节级确定。靶标
> R014（`basalt 6t4w`，seed 88421307）已交付。
> 前序：㉙ 反 trace 计时守卫 → ㉘ 碎片源码净化 → B-3 按块解码 → ㉗ 运行时入密 → ㉖ Luau-only
