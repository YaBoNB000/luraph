...
> 最后更新：2026-09-10（**选项B·B-3 状态链式按块解码**：编译器基本块
> 分割 + BSS 按块编码（会话盐钥匙，静态文件无解码材料）+ RT 按块即需
> 即解（帧私有窗口+块缓存）——批量解码器消亡。门禁 161/161 + 350/350
> + 多种子 0 + 轮廓 32/32 + 安全 5/5 + 钥匙 33 组 0 泄漏；fib(25) 2.57s。
> 前序：㉗ 运行时入密 → ㉖ Luau-only → ㉕ 默认即 v15 → ㉔ 守卫钥匙化
> + 环境值绑定 + 换源防污染
